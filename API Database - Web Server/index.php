<?php 
	...
	if(isset($_GET['from'])){
		...
		if(isset($_GET['target'])){
			...
			$sql = "select a.*, b.nama as nama_bagian from rumah_sakit a left join bagian b on a.`bagian` = b.id where a.nama LIKE '%".$target."%'";
		}else{
			$sql = "select a.*, b.nama as nama_bagian from rumah_sakit a left join bagian b on a.`bagian` = b.id"; // query semua data
		}
	}else{
		$sql = "select a.*, b.nama as nama_bagian from rumah_sakit a left join bagian b on a.`bagian` = b.id limit 20";
	}
